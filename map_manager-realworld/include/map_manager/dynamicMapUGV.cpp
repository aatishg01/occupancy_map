...
namespace mapManager{
	...
	void dynamicMapUGV::getDynamicObstacles(std::vector<Eigen::Vector3d>& obstaclePos,
        ...
        for (size_t i=0 ; i<this->lidarBoxes_.size() ; ++i){
            if (this->isInFov(this->lidarBoxes_[i], this->rangeSensorPosition_, this->bodyOrientation_)){
                continue;
            }
            else{
	...
	void dynamicMapUGV::publish3dBox(const std::vector<box3D>& boxes, const ros::Publisher& publisher, double r, double g, double b) {
        ...
        for(size_t i = 0; i < boxes.size(); i++){
			if (this->isInFov(boxes[i], this->rangeSensorPosition_, this->bodyOrientation_)){ // only show lidar box when they are out of fov
				continue;
			}
            // visualization msgs
